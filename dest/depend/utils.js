...
$.parseURL = function(a) {
    ...
    var b = /^((?:([A-Za-z]+):(\/{0,3}))?([0-9.\-A-Za-z]+\.[0-9A-Za-z]+)?(?::(\d+))?)(?:\/([^?#]*))?(?:\?([^#]*))?(?:#(.*))?$/, c = ["url", "origin", "scheme", "slash", "host", "port", "path", "query", "hash"], d = b.exec(a), e = {};
    if (d)
...
$.betweenTime = function(myTime, serverTime) {
    var before = $.buildDate(myTime), serverTime = $.buildDate(serverTime);
...
$.toFixed = function(str, num) {
    ...
    str = parseFloat(str).toFixed(num + 2);
    ...
    if (_index == -1) {
        ...
        var _addstr = str.substring(_index);
        var front = str.substring(0, _index);

        if (_addstr.length > num + 1) {
            _addstr = _addstr.substring(0, num + 1);
        } else {
            for (var i = 0; i < num + 1 - _addstr.length; i++) {
                _addstr += '0';
            }
        }
        return front + _addstr;
    }
}
Date.now = Date.now ||
function() {
